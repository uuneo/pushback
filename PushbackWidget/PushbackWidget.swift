...
struct CryptoWidgetEntryView : View {
    ...
    let prefix = "pb://openPage?page=widget&title="
    var body: some View {
        // MARK: Building Widget UI With Swift Charts
        switch family{
        case .systemSmall:
            SmallSizeWidget(entry: crypto)
                .widgetURL(URL(string: "\(prefix)\(crypto.result.small?.title ?? "")&data=small"))
        case .systemMedium:
            MediumSizedWidget(entry: crypto)
                .widgetURL(URL(string: "\(prefix)\(crypto.result.medium?.title ?? "")&data=medium"))
        case .systemLarge, .systemExtraLarge:
            LargeSizeWidget(entry: crypto)
                .widgetURL(URL(string: "\(prefix)\(crypto.result.large?.title ?? "")&data=large"))
        default:
            LockScreenWidget(entry: crypto)
                .widgetURL(URL(string: "\(prefix)\(crypto.result.large?.title ?? "")&data=lock"))
        }
    }
