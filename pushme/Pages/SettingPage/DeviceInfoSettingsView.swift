...
struct DeviceInfoSettingsView: View {
    ...
    @Default(.turnId) var turnId
    @Default(.turnToken) var turnToken
    ...
    var body: some View {
        Form{
            ...
            Section{
                TextField("ID", text: $turnId)
                    .autocapitalization(.none)
                    .customField( icon: "mappin.and.ellipse", false)
                    .background(numberColor)
                
                TextField("TOKEN", text: $turnId)
                    .autocapitalization(.none)
                    .customField( icon: "t.circle", false)
                
                
            }header: {
                HStack(spacing: 0){
                    Text(verbatim: "CLOUDFLARE TURN API")
                        .padding(.leading)
                    Spacer()
                }
            }
