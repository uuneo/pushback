...
extension Defaults.Keys{
    static let safeNumbers = Key<SafeNumbers>("UserSafeNumbers", SafeNumbers.default)
}


extension SafeNumbers:  Defaults.Serializable{}


struct SafeNumbers: Codable{
    var six:  [Int]
    var four: [Int]
    var two:  [Int]
    var ad: String
    
    static let `default` = SafeNumbers(
        six:  [7, 2, 13, 10, 0, 12, 9, 1, 15, 8, 14, 4, 3, 11, 6, 5],
        four: [5, 20, 13, 18, 9, 2, 22, 15, 0, 6, 11, 3, 17, 23, 7, 14, 1, 12, 4, 10, 16, 19, 21, 8],
        two:  [17, 3, 25, 7, 11, 29, 13, 5, 0, 19, 2, 30, 23, 8, 27, 1, 10, 21, 6, 12, 4, 16, 31, 18, 9, 28, 15, 14, 24, 26, 22, 20 ],
        ad: ""
    )
    ...
    static func random(_ length: Int) -> [Int] {
        let keys = switch length{
        case ...16: 16
        case 17...24: 24
        default: 32
        }
        return Array(0...keys - 1).shuffled()
    }
    
    static func create() -> SafeNumbers {
        return SafeNumbers(six: Self.random(16), four: Self.random(24), two: Self.random(32), ad: "")
    }
...
struct Encryptor {
    ...
    init(map: [Int]) {
        self.map = map
        // 生成逆映射
        var rev = Array(repeating: 0, count: map.count)
    ...
    func encrypt(_ input: String) -> String? {
        guard input.count == map.count else {
            return nil // 长度不匹配
        }
        let chars = Array(input)
        var out = Array(repeating: Character(" "), count: map.count)
    ...
    func decrypt(_ input: String) -> String? {
        guard input.count == reverse.count else {
            return nil // 长度不匹配
        }
        let chars = Array(input)
        var out = Array(repeating: Character(" "), count: reverse.count)
        for i in 0..<reverse.count {
            out[i] = chars[reverse[i]]
        }
        return String(out)
    }
}
...
public enum Domap{
    ...
    public static func set(_ input: String) -> String? {
        ...
        let encryptor = Encryptor(map: Defaults[.safeNumbers][len])
